...
describe('Gemini usage trend chart', () => {
  beforeEach(() => {
    ...
    getGeminiUsageTrends.mockResolvedValue(googleFixture);
  });

  it('shows Google-like filters and identifies near-real-time Cloud Monitoring data', async () => {
    render(
      <LanguageProvider>
    ...
    expect(await screen.findByText('Google Cloud Monitoring')).toBeInTheDocument();
    ...
    expect(screen.getByText(/có thể trễ tối đa khoảng 150 giây/i)).toBeInTheDocument();
  ...
  it('forces a fresh request and animates the manual refresh action', async () => {
    ...
    await screen.findByText('Google Cloud Monitoring');
