...
const QuizzesListPage = () => {
  ...
  const initialTab = searchParams.get('tab') === 'leaderboard' ? 'leaderboard' : 'quizzes';
  ...
  useEffect(() => {
    ...
    } else if (tabParam !== 'leaderboard' && activeTab === 'leaderboard' && !tabParam) {
  ...
  const [manageFilter, setManageFilter] = useState('all'); // 'all' | 'public' | 'private'
  ...
  const loadManagedQuizzes = async () => {
    try {
      ...
      console.error("Lỗi tải danh sách quản lý đề thi:", err);
    } finally {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-slate-900 transition-colors duration-300" style={{ fontFamily: "'Outfit', sans-serif" }}>
      ...
      <main className="flex-1 max-w-6xl w-full mx-auto px-4 py-28 flex flex-col items-center">
        ...
        {/* Navigation Tabs: Quizzes List vs Global Leaderboard */}
        <div className="flex items-center justify-center p-1.5 bg-slate-200/80 dark:bg-slate-800/80 rounded-2xl mb-10 max-w-md w-full shadow-inner border border-slate-300/40 dark:border-slate-700/60">
          <button
            type="button"
            onClick={() => handleTabChange('quizzes')}
            className={`flex-1 flex items-center justify-center gap-2 py-2.5 px-4 rounded-xl text-xs sm:text-sm font-black transition-all duration-200 cursor-pointer ${
              activeTab === 'quizzes'
                ? 'bg-white dark:bg-slate-700 text-smart-indigo dark:text-indigo-400 shadow-sm'
                : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200'
            }`}
          >
            <FiGrid className="text-base" />
            <span>{t('Danh sách đề thi')}</span>
          </button>
          <button
            type="button"
          ...
          >
            ...
            <span>{t('Bảng xếp hạng')}</span>
            <span className="px-1.5 py-0.5 rounded-md bg-amber-100 dark:bg-amber-950/60 text-amber-700 dark:text-amber-300 text-[10px] font-black border border-amber-300 dark:border-amber-700">
              TOP 20
            </span>
          </button>
        </div>
        ...
        {activeTab === 'leaderboard' ? (
          <GlobalLeaderboardSection onGoToQuizzes={() => handleTabChange('quizzes')} />
        ) : (
          <>
            {/* Quizzes List Header */}
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center w-full max-w-4xl mb-8 gap-4">
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto flex-wrap">
                <button
                  type="button"
                  onClick={() => handleTabChange('leaderboard')}
                  className="flex-1 sm:flex-initial flex items-center justify-center space-x-1.5 px-3.5 py-2.5 bg-amber-50 hover:bg-amber-100 dark:bg-amber-950/40 dark:hover:bg-amber-900/60 text-amber-700 dark:text-amber-300 font-bold text-xs uppercase rounded-xl border border-amber-200 dark:border-amber-800 tracking-wider active:scale-95 transition-all shadow-sm cursor-pointer"
                >
                  <FiAward className="text-sm text-amber-500" /> <span>{t('Bảng xếp hạng')}</span>
                </button>
      ...
      {/* Modal Quản lý Đề thi & Mã PIN cho Giảng viên / Admin */}
      {showManageModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-[9999] p-4 sm:p-6 animate-fade-in">
          <div className="bg-white dark:bg-slate-800 rounded-3xl max-w-4xl w-full max-h-[90vh] flex flex-col shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden">
            {/* Modal Header */}
            <div className="p-5 sm:p-6 border-b border-slate-100 dark:border-slate-700 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/40">
            ...
            <div className="p-4 sm:p-6 border-b border-slate-100 dark:border-slate-700/60 flex flex-col sm:flex-row gap-4 justify-between items-center bg-white dark:bg-slate-800">
              {/* Search input */}
            ...
            <div className="p-4 sm:p-6 overflow-y-auto flex-1 space-y-4 max-h-[60vh]">
              {loadingManaged ? (
                ...
                <div className="text-center py-12 text-slate-400 font-semibold text-xs">
                  Chưa có đề thi nào được khởi tạo trong hệ thống.
                </div>
              ) : (
                managedQuizzes
                  .filter(q => {
                      ...
                      <div className="flex sm:flex-col gap-2 w-full sm:w-auto justify-end border-t sm:border-t-0 pt-3 sm:pt-0 border-slate-100 dark:border-slate-800">
                        <button
                          ...
                          onClick={() => {
                            ...
                            navigate(`/quizzes/play/${q.quiz_id}`);
                          }}
