...
const testConnection = async () => {
  try {
    ...
    // Tự động đồng bộ cấu trúc: Đảm bảo các cột metadata lưu trữ bền vững tồn tại (TASK-DURABLE-LESSON-MEDIA-PIPELINE-01)
    try {
      await client.query(`
        ALTER TABLE lessons 
      ...
      `);
      console.log('✅ Tự động đồng bộ: Đảm bảo metadata lưu trữ bền vững (Durable Media Pipeline) tồn tại thành công');
    } catch (migErr) {
      console.warn('⚠️ Cảnh báo tự động đồng bộ durable media storage:', migErr.message);
    }
